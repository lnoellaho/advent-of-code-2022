    ...
    for input_line in generate_input_lines(
        ...
        if should_create_stacks:
            ...
            for _ in range(0, number_of_crates_to_move):
                try:
                    # popping individual items on a deque is more efficient than removing multiple items or slicing a list
                    moving_crate = crate_stacks[stack_to_move_from].pop()
                    crates_to_move.appendleft(moving_crate)

                except IndexError:
                    pass

            crate_stacks[stack_to_move_to].extend(crates_to_move)
